...
void display(struct processes process[],int no_p,float avgw,float avgt)
{
	...
	for(i=0;i<no_p;i++)	
	{
		printf("\n   %c         %d         %d            %d            %d          %d",process[i].name,process[i].arrival,
		process[i].burst,process[i].completion,process[i].turn,process[i].waiting);
	}
...
{
	int time=0,max=999,i,j,z,flag=1;
	...
	for(time=process[0].arrival;time<total_burst;time++)
	{
		...
		for(i=0;i<no_p;i++)
		{
			if(process[0].arrival<=time && process[i].processed==0)
			flag=0;
		}
